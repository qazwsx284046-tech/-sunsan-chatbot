...
st.title("💊 마음약국 처방전")
st.caption(
    "창세기부터 요한계시록까지 성경파노라마 말씀과 타미드(TAMID) 숏츠 영성"
    " 기도 처방전입니다."
)
st.markdown("---")
...
# (준비되신 전체 스크립트나 강의안을 아래에 붙여넣으시면 됩니다)
# =========================================================
PANORAMA_DATA = """
...
if "messages" not in st.session_state:
    st.session_state.messages = [{
        ...
        "content": (
            ...
            "성경파노라마의 말씀과 **타미드 숏츠 핵심 영성**을 담은 맞춤"
            " 기도 처방전을 전해드립니다."
        ),
    }]
...
if user_input:
    ...
    아래 제공된 [성경파노라마 데이터]와 [타미드 노트 숏츠 영성]을 기준으로 성도의 고민에 가장 알맞은 구절과 기도 처방전을 조제해줘.
    ...
    [답변 작성 가이드]
    1. 타미드 숏츠 콘텐츠처럼 한눈에 들어오는 강력하고 명확한 핵심 한 줄 메시지를 선두에 배치할 것.
    2. 불필요한 길고 복잡한 해설은 배제하고, 고민에 꼭 맞는 성경 구절 본문 전체를 보여줄 것.
    3. 타미드 기도 노트의 5단계(감사/찬양 -> 회개 -> 말씀입각 -> 간구 -> 선포/다짐)가 선명하게 드러나는 짧고 강력한 기도문을 작성할 것.

    [출력 구조]

    🎬 **오늘의 타미드 숏츠 한 줄 선포**
    "(숏츠 타이틀처럼 마음을 깨우는 짧고 강렬한 메시지 1문장)"
    ...
    💊 **타미드(TAMID) 기도 처방전**
    (감사·회개·말씀입각·간구·선포가 반영된 3~4줄의 기도문)
    """
